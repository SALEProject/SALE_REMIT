<?php
	...
	class TbitnovaDesktop extends TDesktop
	{
		function OnLoad()
		{
			TQuark::instance()->viewports = Array();
			TQuark::instance()->viewports[0] = 'bitnovaDesktop_viewport';
			TQuark::instance()->currentViewport = 'bitnovaDesktop_viewport';
			
			$ds = new TDataSet();
			$ds->loadXML('data_qdesktopmenu.xml');
			...
			foreach ($rows0 as $row0)
			{
				...
				if (count($rows1) == 0)
				{
					$pm->OnClick = 'MenuItemClick';
					$pm->Data = $row0['Command'];
				}
		...
		function generateHTML()
		{
			...
			$class = '';
			if ($this->Theme != '') $class = $this->Theme.'_TbitnovaDesktop';
			...
			return	//'<form id="'.$this->Name.'" action="" method=""> '.
					...
					'<div class="'.$class.'_Menu"> '.
					'	<table style="width: 100%; font-size: inherit;" cellpadding="0" cellspacing="0" border="0"> '.
					'		<tr> '.
					'			<td width="140px"><img src="themes/bitnova/logo.png"/><span style="position: relative; top: -2px; width: 80px; padding-left: 0px; font-size: 14px; text-shadow: 0 0 1px #999999;">.quark</span></td> '.
					'			<td>'.$s.'</td> '.
					'			<td style="width: 80px; text-align: center;"><span id="qdesktop_time">time</span></td> '.
					'		</tr> '.
					'	</table> '.
					'</div>'."\n".
		...
		function MenuItemClick($sender)
		{
			...
			if ($url == null) return;
			//TQuark::instance()->addAjaxStack('', 'alert', $url);
				
			//TQuark::instance()->addAjaxStack('', 'alert', $this->contextID);
			//TQuark::instance()->getContext($this->contextID);			
			TQuark::instance()->contextManager->launchApplication($url);
