<?php
	...
	class TDataGrid extends TCustomGrid
	{
		...
		function set_DataSet($value)
		{
			...
			//  set rows
			$j = 0;
			foreach ($this->FDataSet->Rows as $i => $row)
			{
				$j++;
				$i = 0;
				if($row != null){
		...
		function deleteRow($rowIndex, $rowID)
		{
			...
			$this->deleteInternalRow($rowIndex, $rowID);		
			//$this->FDataSet->deleteRow($rowID);	
			$this->FDataSet->deleteRow($rowIndex);
		}
		...
		function reDrawPagination(){
			...
			$script = 	'var obj = getJSform(\''.$this->Parent->id.'\').'.$this->Name.';'."\n".
		...
		protected function generateFilter()
		{
			...
			$html = '<div id="'.$this->id.'_filterWrap" class="'.$class.'_filter">'.
					'	<input type="text"  oninput="getJSform(\'%parent%\').'.$this->Name.'.filterGrid(event, \''.$this->Parent->id.'\', \''.$this->OnFilter.'\', \''.$class.'\', \''.$this->RowsPerPage.'\');" id="'.$this->id.'_filter"></input>'.
		...
		function generateJS()
		{
			...
			$js.=	'getJSform(\'%parent%\').'.$this->Name.'_onclick = function(id_item) '."\n".
					'{ '."\n".
